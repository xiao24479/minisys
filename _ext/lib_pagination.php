...
require_once(ROOT_PATH."_ext/lib_file.php");
...
$sqlcount_='select count(*) from ('.$pagenavigate_sql.') As T';

//die($sqlcount_);

$sqlcount=isset($pagenavigate_countsql)? $pagenavigate_countsql :$sqlcount_;
...
$firstTag='';	
$firstTag= ($toPageNo<=1) ? '第一页' : '<a href=\''.$submiturl.'?j=1&tpg=1'.$pagenavigate_para.'\'>第一页</a>';
$preTag= ($toPageNo<=1) ? '上一页' : '<a href=\''.$submiturl.'?j=1&tpg='.($toPageNo-1).$pagenavigate_para.'\'>上一页</a>';

$nestTag= ($toPageNo==$pages) ? '下一页' : '<a href=\''.$submiturl.'?j=1&tpg='.($toPageNo+1).$pagenavigate_para.'\'>下一页</a>';
$lastTag= ($toPageNo==$pages) ? '最后一页' : '<a href=\''.$submiturl.'?j=1&tpg='.$pages.$pagenavigate_para.'\'>最后一页</a>';
...
for($i=1;$i<$pagenoshow;$i++){
	...
	//echo '前:'.$pagnotemp.'<br>';
	if($pagnotemp>0){
		$prePagenoTag='&nbsp;&nbsp;'.'<a href=\''.$submiturl.'?j=1&tpg='.($toPageNo-$i).$pagenavigate_para.'\'>'.($toPageNo-$i).'</a>'.$prePagenoTag;
		}
	else break;
}
...
for($i=1;$i<$pagenoshow;$i++){
    $pagnotemp=$toPageNo+$i;
	if($pagnotemp<=$pages)
		$nextPagenoTag=$nextPagenoTag.'&nbsp;&nbsp;'.'<a href=\''.$submiturl.'?j=1&tpg='.($toPageNo+$i).$pagenavigate_para.'\'>'.($toPageNo+$i).'</a>';
	else break;
}
