...
class cls_mysql
{
    ...
    var $cache_data_dir = '_cache/caches';
    var $root_path      = '';
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        if ($pconnect)
        {
            if (!($this->link_id = @mysql_pconnect($dbhost, $dbuser, $dbpw)))
            {
                if (!$quiet)
                {
                    $this->error_msg("Can't pConnect MySQL Server($dbhost)!");
                }
        ...
        else
        {
            ...
            if (!$this->link_id)
            {
                if (!$quiet)
                {
                    $this->error_msg("Can't Connect MySQL Server($dbhost)!");
                }
        ...
        if ($dbname)
        {
            if (mysql_select_db($dbname, $this->link_id) === false )
            {
                if (!$quiet)
                {
                    $this->error_msg("Can't select MySQL database($dbname)!");
                }
    ...
    function query($sql, $type = '')
    {
        ...
        {
            $logfilename = $this->root_path . $GLOBALS['_CFG']['db_logfile'] . $this->dbhash . '_' . date('Y_m_d') . '.log';
            $str = $sql . "\n\n";
    ...
    function error_msg($message = '', $sql = '')
    {
        if ($message)
        {
            echo "<b>System Error</b>: $message\n\n";
        }
        else
        {
...
/* 仿真 Adodb 函数 */
    function select_limit($sql, $num, $start = 0)
    {
        if ($start == 0)
        {
            $sql .= ' LIMIT ' . $num;
    ...
    function get_one_cached($sql, $cached = 'FILEFIRST')
    {
        ...
        if (!$cachefirst)
        {
            return $this->get_one($sql, true);
        }
        else
        {
            $result = $this->get_sql_cache_data($sql, $cached);
        ...
        $arr = $this->get_one($sql, true);

        if ($arr !== false && $cachefirst)
        {
            $this->set_sql_cache_data($result, $arr);
        }
    ...
    function get_row_cached($sql, $cached = 'FILEFIRST')
    {
        $sql = trim($sql . ' LIMIT 1');
    ...
    function get_col_cached($sql, $cached = 'FILEFIRST')
    {
        $cachefirst = ($cached == 'FILEFIRST' || ($cached == 'MYSQLFIRST' && $this->platform != 'WINDOWS')) && $this->max_cache_time;
        if (!$cachefirst)
        {
    ...
    function auto_execute($table, $field_values, $mode = 'INSERT', $where = '', $querymode = '')
    {
    ...
    function auto_replace($table, $field_values, $update_values, $where = '', $querymode = '')
    {
        $field_descs = $this->get_all('DESC ' . $table);
    ...
    function set_max_cache_time($second)
    {
        $this->max_cache_time = $second;
    }

    function get_max_cache_time()
    {
        return $this->max_cache_time;
    }

    function get_sql_cache_data($sql, $cached = '')
    {
        $sql = trim($sql);
    ...
    function set_sql_cache_data($result, $data)
    {
