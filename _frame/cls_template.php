...
class cls_template
{
    ...
    function display($filename, $cache_id = '')
    {
		...
		//echo "<br> the cache id is:".$cache_id;
    ...
    {
		//prcess the plugin ..//
		global $_MODULE,$_ACTION,$PLUGINS;

		/*实现插件功能*/
		if(is_array($PLUGINS) && array_key_exists($_MODULE."_".$_ACTION,$PLUGINS) )
		{
			$plugsdata = $PLUGINS[$_MODULE."_".$_ACTION];
			foreach($plugsdata as $fn_=>$fu_ )
			{
				if(is_file(APP_PATH."plugin/".$fn_))
				{
					include_once(APP_PATH."plugin/".$fn_);
//					if(function_exists($fu_))
//						call_user_func($fu_,&$this);
					if(function_exists($fu_)) {
						call_user_func_array($fu_,array(&$this));
					}
				}
			}
		}
		//end of process plugin...
		...
		//echo $this->direct_output;
		//echo "<br>";

            if ($this->direct_output)
            {
				//echo "<br> direct output the page not complete!";
    ...
    function make_compiled($filename)
    {
		...
		//change by jean. 避免名称相同的模板出现问题。
        //$name = $this->compile_dir . '/' . basename($filename) . '.php';
		$temp=md5($filename);
		$name = $this->compile_dir  . basename($filename)."_".$temp. '.php';
...
//					var_dump($tag);
                ...
                case 'include':
					//echo "the tag is:".$tag; echo "end the tag,the game id is:".get_val("game_id"); echo "<br>";
                    $t = $this->get_para(substr($tag, 8), 0);


					$filename = $t['file'];
					//echo "the firest filename is:".$filename."<br>";
					$intpose = strrpos($filename,'<',0);
						//	echo "the int pose  is:".$intpose."<br>";
					if($intpose){
						$intsecpose = strrpos($filename,'>',0);
						//	echo "the int pose  is:".$intsecpose."<br>";
						if($intsecpose){
							$varname=substr($filename,$intpose+2,($intsecpose-$intpose-2));
							//echo "the var name  is:".$varname."<br>";
							//print_r($this->_var);
							$varvalue = $this->_var[$varname];
							//echo "the data value is:".$varvalue."<br>";
							//替换。
							$filename=str_replace("<$"."$varname".">","$varvalue",$filename);
						}
					}

				//	echo "the last filename is:".$filename."<br>";
                    return '<?php echo $this->fetch(' . "'$filename'" . '); ?>';
                    break;
                case 'insert_scripts':
	...
	function get_directory_178avatar($uid){
		$hex = dechex($uid);
            ...
            foreach ($moddb AS $key => $mod)
            {
                ...
                switch ($s[0])
                {
                    case 'escape':
						...
						elseif ($s[1] == 'dehtml')	 // 自定义增加 html反转义
                        {
                            $p = 'htmlspecialchars_decode(' . $p . ')';
                        }
						elseif ($s[1] == 'stripdehtml')	// 自定义增加html反转义和单引反转义
                        {
							$p = 'stripslashes(htmlspecialchars_decode(' . $p . '))';
                        }
                        else
                        {
                            $p = 'htmlspecialchars(' . $p . ')';
                  ...
                  case 'dump':
                    ...
                    case 'strip_tags':
                       // $p = 'strip_tags(' . $p . ')';
                        ...
                        break;
					case 'avatar178':
						$p ="'".$this->get_directory_178avatar($p)."'";
						break;
                    default:
                        # code...
                        break;
...
//		print_r($tokens);
        ...
        {
			// 针对strpos 修改。
			if( isset($tokens[2]) && $tokens[2]=="strpos" ) {
				return '<?php if( strpos(' . $tokens[0] . ',' . $tokens[4] .' ) !== false ): ?>';
			}
