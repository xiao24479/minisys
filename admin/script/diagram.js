...
{ 
  ...
  { if (ii<365*24*60*60*1000)
    { vv_date.setTime(vv+15*24*60*60*1000);
      ...
      if (ttype==5) return(mm+"/"+dd);//You can add your own date format here
...
{ var x0,y0,i,j,itext,l,x,y,r,u,fn,dx,dy,xr,yr,invdifx,invdify,deltax,deltay,id=this.ID,lay=0,selObj="",divtext="",ii=0,oo,k,sub,sshift;
  ...
  {   
    ...
    { 
      ...
      if (deltax!=0) 
		  this.MaxGrids = Math.floor( Math.abs((this.xmax - this.xmin) / deltax) ) + 2;
      else 
		  this.MaxGrids=0;
	  ...
	  for (j=this.MaxGrids; j>=-2; j--)
      {
        ...
        { 
          ...
          if ((this.XGridColor)&&(x0>this.left)&&(x0<this.right)) 
			  divtext+="<div id='"+this.ID+"i"+eval(ii++)+"' style='position:absolute; left:"+x0+"px; top:"+eval(this.top+1)+"px; z-index:"+this.zIndex+"; width:1px; height:"+eval(this.bottom-this.top-1)+"px; background-color:"+this.XGridColor+"; font-size:1pt'></div>";
        }
      }
    }
  }
...
{ this.ID="Line"+_N_Line; _N_Line++; _zIndex++;
  ...
  { 
    ...
    { ccr=0;
      ...
      { 
		  ccl=ccr;
        while ( (2 * ccr * hh <= (2 * cct + 1) * ww) && (ccr <= ww) ) 
			ccr++;
