...
class smallprogram1 extends cls_base{
	...
	public function message(){
		...
		$this->template->display("smallprogram1/message.htm");
	...
	function gain_data(){
		...
		$user_data=getUser();
		$id = get_data('id');//访客id
		$add_time=date('Y-m-d',strtotime("-60 day"));
		...
		if($row){
			make_json_result($this->template->fetch('smallprogram1/gain_data.htm'),1,array());
		}else{
			make_json_result($this->template->fetch('smallprogram1/gain_data.htm'),0,array());
	 ...
	 */
	 public function check_vistor2(){
		...
		make_json_result($this->template->fetch("smallprogram1/check_vistor.htm"),1, array());
	...
	**/
	...
	function dialogue(){
		...
		make_json_result($this->template->fetch("smallprogram1/dialogue.htm"),1,array());
	}
